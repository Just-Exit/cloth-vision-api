...
from cloth_vision_api.ports.outbound import IdentityRepository, PasswordManager, TokenManager
...
class AuthService:
    ...
    def signup(self, email: str, password: str, nickname: str) -> tuple[User, str, int]:
        ...
        if self.identity_repository.get_user_by_email(normalized_email):
            raise ConflictError("이미 사용 중인 이메일입니다.")

        user = self.identity_repository.add_user(User(email=normalized_email, nickname=nickname))
        ...
        self.identity_repository.add_auth_identity(identity)
    ...
    def login(self, email: str, password: str) -> tuple[User, str, int]:
        normalized_email = email.strip().lower()
        identity = self.identity_repository.get_auth_identity(
            AuthProvider.LOCAL.value, normalized_email
        )
        if (
            not identity
        ...
        user = self.identity_repository.get_user(identity.user_id)
        if not user:
